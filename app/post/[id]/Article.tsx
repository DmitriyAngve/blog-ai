...
const Article = ({
  ...
  // function that's going to call backend and generate AI content

  const postAiContent = async () => {
    // chain/focus/run - is a part of TipTap
    editor
      ?.chain()
      .focus()
    ...
    // grap the "response" tp get back
    const data = await response.json();

    // setting up a chain and focus to do setContent to the information that we get from the backend
    editor.chain().focus().setContent(data.content).run();
    // and again set the content to be data content
